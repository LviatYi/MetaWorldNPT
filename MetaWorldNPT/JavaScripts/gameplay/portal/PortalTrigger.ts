...
export default class PortalTrigger extends mw.Script {
    ...
    public destination: mw.Vector = mw.Vector.zero;
    ...
    protected transferPlayer(character: Character) {
        ...
        if (character) {
            ...
            if (player) {
                ...
                character.addImpulse(velocity.clone().multiply(mw.Vector.negOne), true);
                ...
                if (this.isRefreshCameraRotation) this._nolan.lookToward(this.endRotation.rotateVector(mw.Vector.forward));

                if (this.isHoldVelocity) {
                    let v = this.isRefreshObjectRotation ?
                        this.endRotation.rotateVector(mw.Vector.forward).multiply(velocity.length) :
                        velocity;
