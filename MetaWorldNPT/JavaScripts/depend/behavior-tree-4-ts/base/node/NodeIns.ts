...
export class NodeIns<C extends Context = Context> implements INodeIns<C> {
    ...
    public run(env: Environment<C, INodeIns<C>>): NodeRetStatus {
        ...
        if (currYieldAt === NOT_YIELD) {
            ...
            NodeIns.error(`child is yield. you must handle this first.`,
                `self: ${this.logTitle}`,
        ...
        if (vars.length > 0) NodeIns.log(`input:`,
            ...vars.map((k, i) =>
                `    ${k}: ${vars[i]}`),
        );
        ...
        if (errored) {
            NodeIns.error(`error occurs when run node.`,
                `node: ${this.logTitle}`,
                errorInfo ?? "");
        ...
        for (let i = 0;
             ...
             i++) {
            NodeIns.log(() => `output: ${this.define.output[i]}: ${ret.out[i]}`);
