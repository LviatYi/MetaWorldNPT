...
//#region Config
    ...
    public setGlobalTipsContainer(container: Constructor<GlobalTipsContainer<unknown>>): this {
        if (!this.checkInClient()) return this;
        if (this.containerValid()) {
            ...
            mw.UIService.hideUI(this._holder);
            this._onlyHiddenTask.destroy();
        }

        this._holder = mw.UIService.create(container);
        if (!this._holder ||
    ...
    public setBubbleWidget(widget: Constructor<BubbleTipWidget<unknown>>): this {
        ...
        this._bubbleWidgetPool = new ObjectPool<RecyclableBubbleWidget>(
            {
                generator: () => {
                    const widget = mw.UIService.create(this._bubbleWidgetCls);
...
//#endregion ⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠐⠒⠒⠒⠒⠚⠛⣿⡟⠄⠄⢠⠄⠄⠄⡄⠄⠄⣠⡶⠶⣶⠶⠶⠂⣠⣶⣶⠂⠄⣸⡿⠄⠄⢀⣿⠇⠄⣰⡿⣠⡾⠋⠄⣼⡟⠄⣠⡾⠋⣾⠏⠄⢰⣿⠁⠄⠄⣾⡏⠄⠠⠿⠿⠋⠠⠶⠶⠿⠶⠾⠋⠄⠽⠟⠄⠄⠄⠃⠄⠄⣼⣿⣤⡤⠤⠤⠤⠤⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄
    ...
    private refreshOnlyTipsHiddenTimer(duration?: number) {
        ...
        this._onlyTipsHiddenTimer = mw.setTimeout(() => {
                try {
                    this._holder?.tipHidden?.();
                } catch (e) {
                    Log4Ts.error(GlobalTips, `error occurs in tipHidden`, e);
                }
                (!this._useCustomOnlyEffect) && this._onlyHiddenTask.continue();
            },
